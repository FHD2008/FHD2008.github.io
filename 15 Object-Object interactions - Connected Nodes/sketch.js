...
function draw() {
  ...
  for (let n of nodes){
    // looping by item is good here as we don't plan on
    //deleting any objects from the array
    n.move();
    n.connect(nodes);
    n.display();
  }
...
class csNode{
  ...
  constructor(x,y){ //called once per object created
    //properties related to postion/rendedering
  ...
  connect(nodeArray){
    ...
    for(let n of nodes){
      ...
      if(n !== this){ //make sure not to compare itself
